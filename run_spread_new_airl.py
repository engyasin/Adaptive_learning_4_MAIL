...
def main(num_of_exp_eps=32,test_mode=0):
    ...
    expert = expert_p(n_agents=n_agents)
    ...
    ## collective
    try:
        rollouts_coop = types.load(f'rollouts_coop_{num_of_exp_eps}_{n_agents}.npz')
    except:
        # file not saved
        ...
        types.save(f'rollouts_coop_{num_of_exp_eps}_{n_agents}.npz',rollouts_coop)
    ...
    learner_.save('bc_init_s_{}'.format(n_agents))
    ...
    if bc_:
        ...
        try:
            learner_ = PPO.load('bc_trained_{}_{}'.format(n_agents,num_of_exp_eps))
            bc_trainer = bc.BC(
                observation_space=venv.observation_space,
                action_space=venv.action_space,
                demonstrations=transitions,
    ...
    # NOTE warm startup:
    learner_.save('bc_trained_{}_{}'.format(n_agents,num_of_exp_eps))

    if warm_start:
        learner = PPO.load('bc_trained_{}_{}'.format(n_agents,num_of_exp_eps))
    else:
    ...
    shaped_reward_net = Shaped_R_Blended(venv.observation_space, venv.action_space,n_agents=n_agents,lr=0.005)
    ...
    results_file = './single_gail_hot/{0}_single_gail_ds_{1}_agents_{2}_test_{3}.txt'.format(['cold','warm'][warm_start],
    ...
    now_ = num_of_exp_eps
    learner.save('trainer_{}_test_{}'.format(now_,test_mode))
    th.save(shaped_reward_net.state_dict(), 'ens_rewards_{}_test_{}'.format(now_,test_mode))
