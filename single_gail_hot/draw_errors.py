...
def read_data(data_size=16,test_mode=0):
    ...
    with open(f'warm_single_gail_ds_{data_size}_agents_3_test_{test_mode}.txt') as f:
        ...
        if 'GAIL' in data[1]:
            ...
            #random and bc
            results['Random'] = get_errors(data[0])
            for j in range(bc_epochs):
                results['BC'].append(get_errors(data[j+1],step=j))
            data = data[bc_epochs+1:]
...
def plot_results(results, data_size=16):
    ...
    plt.subplot(2,1,1)
    #plt.title('ADE')
    N = int(len(full[:,0])*0.30)#30%
    
    ade_full = np.mean(full[-N:,3])
    plt.plot(full[:,0],rev_accum(full[:,3]),'-',color='green',label=f'Full-ADE ',linewidth=3)
    ...
    plt.plot(full[:,0],rev_accum(full[:,4]),'-',color='green',label=f'Full-FDE  ',linewidth=3)
    #plt.fill_between(full[:,0],full[:,3]-full[:,2]/scale_std,full[:,3]+full[:,2]/scale_std,color='gray',alpha=0.2)
    ...
    if False:
        ...
        plt.plot(full[:,0],rev_accum(full[:,1]),'-',color='green',label=f'Full-R  ',linewidth=3)
        ...
        plt.xticks([1, smallest_size*3, smallest_size*5])
        plt.legend()
        plt.xlabel('Number of evaluation steps counted in the average',fontsize=12)
...
def plot_results__(results, data_size=16):
    ...
    plt.legend()
    plt.xlabel('Number of evaluation steps counted in the average',fontsize=14)
    plt.ylabel('Mean ADE')
    #plt.xlim([0,880])
    plt.grid()
    plt.show()
...
if __name__ == '__main__':
    ...
    Ade_Fde = []
    #ds = 256
    for ds in [10,20,80,160]:#,16,256,1024]:
        results = []
        for i in range(3):
            results.append(read_data(data_size=ds,test_mode=i))

        print(ds)
        print(plot_results(results,data_size=ds))
